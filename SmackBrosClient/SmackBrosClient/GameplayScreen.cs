...
namespace SmackBrosClient
{
    public partial class GameplayScreen:Screen
    {
        ...
        protected void OnUnload(ref OpenGL gl)
        {
        ...
        private bool LoadModelAsset(string path, ref OpenGL gl)
        {
            ...
            if (m_model != null)
            {
                Get_Bounding_Box(m_sceneMin, m_sceneMax, ref gl);
        ...
        void Get_Bounding_Box(Vector3 min, Vector3 max, ref OpenGL gl)
        {

            Matrix4x4 trafo = Matrix4x4.Identity;
            min.X = min.Y = min.Z = 1e10f;
            max.X = max.Y = max.Z = -1e10f;
            Get_Bounding_Box_For_Node(m_model.RootNode, min, max, trafo, ref gl);
        }
        void Get_Bounding_Box_For_Node(Node nd, Vector3 min, Vector3 max, Matrix4x4 trafo, ref OpenGL gl)
        {
            ...
            {
                Get_Bounding_Box_For_Node(nd.Children[k], min, max, trafo, ref gl);
            }
            trafo = prev;
        }
